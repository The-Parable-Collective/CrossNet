...
internal static class NativeSocket
{
    ...
    {
        return UnixMode
            ? UnixSock.recvfrom(socketHandle, pinnedBuffer, length, 0, socketAddress, ref socketAddressSize)
            : WinSock.recvfrom(socketHandle, pinnedBuffer, length, 0, socketAddress, ref socketAddressSize);
    }
    ...
    {
        return UnixMode
            ? UnixSock.sendto(socketHandle, pinnedBuffer, length, 0, socketAddress, socketAddressSize)
            : WinSock.sendto(socketHandle, pinnedBuffer, length, 0, socketAddress, socketAddressSize);
    }
    ...
    public static SocketError GetSocketError()
    {
        ...
        return UnixMode
            ? NativeErrorToSocketError.TryGetValue(error, out SocketError err)
                ? err
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static short GetNativeAddressFamily(IPEndPoint endPoint)
    {
        return UnixMode
            ? (short)(endPoint.AddressFamily == AddressFamily.InterNetwork ? AF_INET : AF_INET6)
            : (short)endPoint.AddressFamily;
    }
}